...
App = {
	init: function(app,server) {
		...
		app.post('/agents',function(req,res) {
			...
			if (req.body.kuni)
			{
				App.using('db').model('bpclight','SELECT batiments.LibBatC, unites.Kets KEts, batiments.GPS, agents.* FROM ((agents LEFT OUTER JOIN subdis ON (agents.Ksub = subdis.Ksub)) LEFT OUTER JOIN batiments ON (agents.Kbat = batiments.Kbat)) LEFT OUTER JOIN unites ON (agents.Kuni = unites.Kuni) WHERE actif=1 and unites.kuni='+req.body.kuni+' order by nom,prenom',function(err,o) {
					res.end(JSON.stringify(o,null,4));
				});
				return;
			};
