Agents = {
	...
	{
		Agents.using('db').query('bpclight','SELECT position.Position FROM bpclight.ageetat ageetat INNER JOIN bpclight.position position ON (ageetat.Kpst = position.Kpst) WHERE ageetat.Keta = '+o+' ORDER BY ageetat.DatEta desc',cb);
	},
