Agents = {
	...
	{
		var db=Agents.using('db');
		db.query('bpclight','SELECT position.Position FROM bpclight.ageetat ageetat INNER JOIN bpclight.position position ON (ageetat.Kpst = position.Kpst) WHERE (ageetat.Kage = '+o+' and ageetat.DatCre<=NOW()) ORDER BY ageetat.DatCre desc',cb);
	},
